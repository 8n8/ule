...
{
	if (region.bw.aD === region.bS.aD)
	{
		return 'on line ' + region.bw.aD;
	}
	return 'on lines ' + region.bw.aD + ' through ' + region.bS.aD;
}
...
function _Browser_visibilityInfo()
{
	return (typeof _VirtualDom_doc.hidden !== 'undefined')
		? { cT: 'hidden', cL: 'visibilitychange' }
		:
	(typeof _VirtualDom_doc.mozHidden !== 'undefined')
		? { cT: 'mozHidden', cL: 'mozvisibilitychange' }
		:
	(typeof _VirtualDom_doc.msHidden !== 'undefined')
		? { cT: 'msHidden', cL: 'msvisibilitychange' }
		:
	(typeof _VirtualDom_doc.webkitHidden !== 'undefined')
		? { cT: 'webkitHidden', cL: 'webkitvisibilitychange' }
		: { cT: 'hidden', cL: 'visibilitychange' };
}
...
var _Http_toTask = F2(function(request, maybeProgress)
{
	return _Scheduler_binding(function(callback)
	{
		...
		xhr.addEventListener('load', function() {
			callback(_Http_handleResponse(xhr, request.bU.a));
		});

		try
		{
			xhr.open(request.c$, request.a6, true);
		}
		catch (e)
		{
			return callback(_Scheduler_fail($elm$http$Http$BadUrl(request.a6)));
		}
...
function _Http_configureRequest(xhr, request)
{
	...
	xhr.responseType = request.bU.b;
	xhr.withCredentials = request.$7;

	$elm$core$Maybe$isJust(request.cx) && (xhr.timeout = request.cx.a);
}
...
function _Http_toResponse(xhr)
{
	return {
		a6: xhr.responseURL,
		dc: { cM: xhr.status, c_: xhr.statusText },
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {bY: fragment, b$: host, c5: path, ce: port_, ci: protocol, cj: query};
	});
...
var $elm$time$Time$State = F2(
	function (taggers, processes) {
		return {ch: processes, cv: taggers};
	});
...
var $elm$http$Http$expectJson = function (decoder) {
	return $elm$http$Http$expectStringResponse(
		function (response) {
			var _v0 = A2($elm$json$Json$Decode$decodeString, decoder, response.bI);
			if (_v0.$ === 1) {
...
var $ohanhi$remotedata_http$RemoteData$Http$createRequest = F5(
	function (config, method, url, successDecoder, body) {
		return $elm$http$Http$request(
			{
				bI: body,
				bU: $elm$http$Http$expectJson(successDecoder),
				bZ: config.bZ,
				c$: method,
				cx: config.cx,
				a6: url,
...
var $ohanhi$remotedata_http$RemoteData$Http$noCacheConfig = _Utils_update(
	...
	{
		bZ: A2($elm$core$List$cons, $ohanhi$remotedata_http$RemoteData$Http$noCache, $ohanhi$remotedata_http$RemoteData$Http$defaultConfig.bZ)
	});
...
var $author$project$Main$init = F3(
	function (flags, url, navKey) {
		return _Utils_Tuple2(
			{
				q: $author$project$Main$NotReady(
					$elm$time$Time$millisToPosix(flags.aU)),
				aE: navKey,
				a6: url
			},
...
var $author$project$Routing$Helpers$HomeRoute = 0;
var $author$project$Routing$Helpers$NotFoundRoute = 2;
var $elm$url$Url$Parser$State = F5(
	function (visited, unvisited, params, frag, value) {
		return {M: frag, Q: params, J: unvisited, A: value, U: visited};
	});
		...
		return $elm$url$Url$Parser$getFirstMatch(
			parser(
				A5(
					...
					_List_Nil,
					$elm$url$Url$Parser$preparePath(url.c5),
					$elm$url$Url$Parser$prepareQuery(url.cj),
					url.bY,
					$elm$core$Basics$identity)));
	});
var $author$project$Routing$Helpers$SettingsRoute = 1;
	...
	if (_v0.$ === 1) {
		...
		return A2(
			...
			A2(
				...
				_Utils_update(
					url,
					{bY: $elm$core$Maybe$Nothing, c5: fragment})));
	}
};
...
var $author$project$Types$Commit = F4(
	function (userName, sha, date, message) {
		return {bO: date, c_: message, db: sha, dm: userName};
	});
...
var $elm$parser$Parser$Advanced$DeadEnd = F4(
	function (row, col, problem, contextStack) {
		return {bN: col, cN: contextStack, cf: problem, co: row};
	});
...
var $elm$parser$Parser$Advanced$fromState = F2(
	function (s, x) {
		return A2(
			...
			A4($elm$parser$Parser$Advanced$DeadEnd, s.co, s.bN, x, s.f));
	});
		...
		while (true) {
			...
			if (_Utils_eq(newOffset, -1)) {
				return A3(
					...
					0,
					{bN: col, f: s0.f, g: s0.g, b: offset, co: row, a: s0.a});
			} else {
...
var $elm$parser$Parser$Advanced$chompWhile = function (isGood) {
	return function (s) {
		return A5($elm$parser$Parser$Advanced$chompWhileHelp, isGood, s.b, s.co, s.bN, s);
	};
};
...
var $elm$parser$Parser$Advanced$token = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, str, s.b, s.co, s.bN, s.a);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		return _Utils_eq(newOffset, -1) ? A2(
			...
			progress,
			0,
			{bN: newCol, f: s.f, g: s.g, b: newOffset, co: newRow, a: s.a});
	};
};
...
var $elm$parser$Parser$DeadEnd = F3(
	function (row, col, problem) {
		return {bN: col, cf: problem, co: row};
	});
var $elm$parser$Parser$problemToDeadEnd = function (p) {
	return A3($elm$parser$Parser$DeadEnd, p.co, p.bN, p.cf);
};
...
var $elm$parser$Parser$Advanced$run = F2(
	function (_v0, src) {
		var parse = _v0;
		var _v1 = parse(
			{bN: 1, f: _List_Nil, g: 1, b: 0, co: 1, a: src});
		if (!_v1.$) {
			var value = _v1.b;
			return $elm$core$Result$Ok(value);
		} else {
...
var $author$project$Types$Stargazer = F3(
	function (login, avatarUrl, url) {
		return {cI: avatarUrl, cZ: login, a6: url};
	});
		...
		switch (msg.$) {
			case 2:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{ay: $krisajenkins$remotedata$RemoteData$Loading, aJ: $krisajenkins$remotedata$RemoteData$Loading}),
...
var $author$project$Routing$Router$updateHome = F2(
	function (model, homeMsg) {
		var _v0 = A2($author$project$Pages$Home$update, homeMsg, model.aB);
		var nextHomeModel = _v0.a;
		var homeCmd = _v0.b;
		return _Utils_Tuple3(
			_Utils_update(
				model,
...
var $author$project$Routing$Router$updateSettings = F3(
	function (sharedState, model, settingsMsg) {
		var _v0 = A3($author$project$Pages$Settings$update, sharedState, settingsMsg, model.aI);
		var nextSettingsModel = _v0.a;
		var settingsCmd = _v0.b;
		var sharedStateUpdate = _v0.c;
		return _Utils_Tuple3(
			_Utils_update(
				model,
				{aI: nextSettingsModel}),
			...
			case 0:
				...
				return _Utils_Tuple3(
					_Utils_update(
						model,
						{
							ak: $author$project$Routing$Helpers$parseUrl(location)
						}),
					$elm$core$Platform$Cmd$none,
					$author$project$SharedState$NoUpdate);
			case 1:
		...
		if (_v0.$ === 1) {
			...
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{
						q: A2($author$project$Main$Ready, nextSharedState, nextRouterModel)
					}),
			...
			case 0:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							q: $author$project$Main$NotReady(time)
						}),
					$elm$core$Platform$Cmd$none);
			case 1:
				var sharedState = _v0.a;
...
var $author$project$Pages$Settings$initModel = {a3: 0};
var $author$project$Routing$Router$init = function (url) {
	var settingsModel = $author$project$Pages$Settings$initModel;
	var _v0 = $author$project$Pages$Home$init;
	var homeModel = _v0.a;
	var homeCmd = _v0.b;
	return _Utils_Tuple2(
		{
			aB: homeModel,
			ak: $author$project$Routing$Helpers$parseUrl(url),
			aI: settingsModel
		},
		A2($elm$core$Platform$Cmd$map, $author$project$Routing$Router$HomeMsg, homeCmd));
};
		...
		switch (webData.$) {
			case 2:
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{q: $author$project$Main$FailedToInitialize}),
					$elm$core$Platform$Cmd$none);
			case 3:
				var translations = webData.a;
				var _v1 = model.q;
				switch (_v1.$) {
					case 0:
						var time = _v1.a;
						var initSharedState = {aU: time, aE: model.aE, dj: translations};
						var _v2 = $author$project$Routing$Router$init(model.a6);
						var initRouterModel = _v2.a;
						var routerCmd = _v2.b;
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									q: A2($author$project$Main$Ready, initSharedState, initRouterModel)
								}),
							A2($elm$core$Platform$Cmd$map, $author$project$Main$RouterMsg, routerCmd));
					case 1:
...
var $rtfeldman$elm_css$Css$prop2 = F3(
	function (key, argA, argB) {
		return A2($rtfeldman$elm_css$Css$property, key, argA.A + (' ' + argB.A));
	});
...
var $rtfeldman$elm_css$Css$row = {bb: 0, az: 0, A: 'row'};
...
var $rtfeldman$elm_css$Css$bold = {F: 0, A: 'bold'};
...
var $rtfeldman$elm_css$Css$none = {V: 0, bH: 0, r: 0, c: 0, j: 0, cU: 0, b2: 0, bi: 0, ag: 0, N: 0, y: 0, e: 0, d: 0, bk: 0, a$: 0, c6: 0, v: 0, a0: 0, c9: 0, an: 0, T: 0, t: 0, i: 0, dl: 0, A: 'none'};
...
var $rtfeldman$elm_css$Css$pointer = {c: 0, A: 'pointer'};
		...
		switch (declaration.$) {
			...
			case 6:
				...
				return $elm$core$String$isEmpty(record.cO) ? _Utils_Tuple2(keyframesByName, declarations) : _Utils_Tuple2(
					A3($elm$core$Dict$insert, record.c1, record.cO, keyframesByName),
					declarations);
			case 7:
				var properties = declaration.a;
				return $elm$core$List$isEmpty(properties) ? _Utils_Tuple2(keyframesByName, declarations) : _Utils_Tuple2(
...
var $rtfeldman$elm_css$Css$Structure$compactStylesheet = function (_v0) {
	...
	return {
		bM: charset,
		cP: $rtfeldman$elm_css$Css$Structure$compactDeclarations(declarations),
		b1: imports,
		cb: namespaces
	};
};
...
var $robinheghan$murmur3$Murmur3$HashData = F4(
	function (shift, seed, hash, charsProcessed) {
		return {X: charsProcessed, ac: hash, R: seed, al: shift};
	});
...
var $robinheghan$murmur3$Murmur3$finalize = function (data) {
	var acc = (!(!data.ac)) ? (data.R ^ A2(
		...
		A2(
			...
			15,
			A2($robinheghan$murmur3$Murmur3$multiplyBy, $robinheghan$murmur3$Murmur3$c1, data.ac)))) : data.R;
	var h0 = acc ^ data.X;
...
var $robinheghan$murmur3$Murmur3$hashFold = F2(
	function (c, data) {
		var res = data.ac | ((255 & $elm$core$Char$toCode(c)) << data.al);
		var _v0 = data.al;
		if (_v0 === 24) {
			return {
				X: data.X + 1,
			...
			return {X: data.X + 1, ac: res, R: data.R, al: data.al + 8};
		}
	});
...
var $rtfeldman$elm_css$Css$Preprocess$Resolve$toStructure = function (_v0) {
	...
	var declarations = $rtfeldman$elm_css$Css$Preprocess$Resolve$extract(
		A2($elm$core$List$concatMap, $rtfeldman$elm_css$Css$Preprocess$unwrapSnippet, snippets));
	return {bM: charset, cP: declarations, b1: imports, cb: namespaces};
};
...
var $rtfeldman$elm_css$Css$Preprocess$stylesheet = function (snippets) {
	return {bM: $elm$core$Maybe$Nothing, b1: _List_Nil, cb: _List_Nil, cs: snippets};
};
...
var $author$project$Pages$Home$ReloadData = {$: 2};
...
var $rtfeldman$elm_css$Css$prop4 = F5(
	function (key, argA, argB, argC, argD) {
		return A2($rtfeldman$elm_css$Css$property, key, argA.A + (' ' + (argB.A + (' ' + (argC.A + (' ' + argD.A))))));
	});
...
var $rtfeldman$elm_css$Css$solid = {r: 0, S: 0, A: 'solid'};
var $rtfeldman$elm_css$Css$transparent = {x: 0, A: 'transparent'};
...
var $author$project$Pages$Home$formatTimestamp = F2(
	function (sharedState, date) {
		var translate = $author$project$I18n$get(sharedState.dj);
		var timeDiff = $elm$time$Time$posixToMillis(sharedState.aU) - $elm$time$Time$posixToMillis(date);
		var seconds = A2(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Html$Styled$text(commit.dm)
						])),
					A2(
					...
					_List_fromArray(
						[
							...
							A2($author$project$Pages$Home$formatTimestamp, sharedState, commit.bO))
						])),
					A2(
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Html$Styled$text(commit.c_)
						]))
				]));
	});
		...
		switch (_v0.$) {
			case 1:
				return $rtfeldman$elm_css$Html$Styled$text(
					A2($author$project$I18n$get, sharedState.dj, 'status-loading'));
			case 2:
				return $rtfeldman$elm_css$Html$Styled$text(
					A2($author$project$I18n$get, sharedState.dj, 'status-network-error'));
			case 3:
				var commits = _v0.a;
				return A2(
					...
					A2(
						...
						A2(
							...
							function (commit) {
								return -$elm$time$Time$posixToMillis(commit.bO);
							},
							commits)));
			default:
				return $rtfeldman$elm_css$Html$Styled$text('');
...
var $rtfeldman$elm_css$Css$borderBox = {a9: 0, aT: 0, A: 'border-box'};
			...
			[
				...
				_List_fromArray(
					[
						...
						$rtfeldman$elm_css$Html$Styled$Attributes$src(stargazer.cI)
					]),
				_List_Nil),
				A2(
				...
				_List_fromArray(
					[
						...
						$rtfeldman$elm_css$Html$Styled$Attributes$href(stargazer.a6)
					]),
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(stargazer.cZ)
					]))
			]));
};
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$rtfeldman$elm_css$Html$Styled$text(
									'↻ ' + A2($author$project$I18n$get, sharedState.dj, 'commits-refresh'))
								]))
						])),
					A2(
					$rtfeldman$elm_css$Html$Styled$div,
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$rtfeldman$elm_css$Html$Styled$text(
											A2($author$project$I18n$get, sharedState.dj, 'commits-heading'))
										])),
							...
							_List_fromArray(
								[
									...
									_List_fromArray(
										[
											$rtfeldman$elm_css$Html$Styled$text(
											A2($author$project$I18n$get, sharedState.dj, 'stargazers-heading'))
										])),
...
var $author$project$Types$Finnish = 1;
var $author$project$Types$FinnishFormal = 2;
...
var $rtfeldman$elm_css$Css$monospace = {L: 0, A: 'monospace'};
...
var $author$project$Pages$Settings$translationRow = function (_v0) {
	var key = _v0.a;
	var value = _v0.b;
	return A2(
			...
			[
				...
				_List_fromArray(
					[
						$rtfeldman$elm_css$Html$Styled$text(
						$elm$core$String$fromInt(
							$elm$time$Time$posixToMillis(sharedState.aU)))
					])),
				A2(
				$rtfeldman$elm_css$Html$Styled$h4,
				_List_fromArray(
				...
				A2(
					...
					$elm$core$Dict$toList(sharedState.dj)))
			]));
};
...
var $author$project$Pages$Settings$selectionButton = F3(
	function (model, language, shownName) {
		var buttonStyles = _Utils_eq(model.a3, language) ? _Utils_ap($author$project$Styles$actionButtonActive, $author$project$Styles$gutterRight) : _Utils_ap($author$project$Styles$actionButton, $author$project$Styles$gutterRight);
		return A2(
			$rtfeldman$elm_css$Html$Styled$button,
			_List_fromArray(
				[
...
var $author$project$Pages$Settings$view = F2(
	function (sharedState, model) {
		var t = $author$project$I18n$get(sharedState.dj);
		return A2(
			$rtfeldman$elm_css$Html$Styled$div,
			_List_Nil,
			_List_fromArray(
				[
					...
					A3($author$project$Pages$Settings$selectionButton, model, 0, 'English'),
					A3($author$project$Pages$Settings$selectionButton, model, 2, 'Suomi (virallinen)'),
					A3($author$project$Pages$Settings$selectionButton, model, 1, 'Suomi (puhekieli)'),
					A2(
					$rtfeldman$elm_css$Html$Styled$h2,
					_List_Nil,
					_List_fromArray(
					...
					_List_fromArray(
						[
							$rtfeldman$elm_css$Html$Styled$Events$onClick(
							$author$project$Pages$Settings$NavigateTo(0)),
					...
					switch (_v0) {
						case 0:
							return A2(
								...
								A2($author$project$Pages$Home$view, sharedState, model.aB));
						case 1:
							return A2(
								$rtfeldman$elm_css$Html$Styled$map,
								$author$project$Routing$Router$SettingsMsg,
								A2($author$project$Pages$Settings$view, sharedState, model.aI));
						default:
							return A2(
...
var $rtfeldman$elm_css$Css$auto = {cF: 0, c: 0, _: 0, aX: 0, cY: 0, ae: 0, G: 0, y: 0, ah: 0, v: 0, a4: 0, ap: 0, t: 0, A: 'auto'};
...
var $author$project$Routing$Router$view = F3(
	function (msgMapper, sharedState, model) {
		...
		var buttonStyles = function (route) {
			return _Utils_eq(model.ak, route) ? $author$project$Styles$styles($author$project$Styles$navigationButtonActive) : $author$project$Styles$styles($author$project$Styles$navigationButton);
		};
		var body = A2(
			$rtfeldman$elm_css$Html$Styled$div,
			_List_fromArray(
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$rtfeldman$elm_css$Html$Styled$Events$onClick(
									$author$project$Routing$Router$NavigateTo(0)),
									buttonStyles(0)
								]),
							_List_fromArray(
								[
							...
							_List_fromArray(
								[
									$rtfeldman$elm_css$Html$Styled$Events$onClick(
									$author$project$Routing$Router$NavigateTo(1)),
									buttonStyles(1)
								]),
							_List_fromArray(
								[
		...
		return {
			...
			cy: title + ' - Elm Shared State Demo'
		};
	});
...
var $author$project$Main$main = $elm$browser$Browser$application(
	{
		...
		c2: $author$project$Main$UrlChange,
		c3: $author$project$Main$LinkClicked,
		dd: function (_v0) {
